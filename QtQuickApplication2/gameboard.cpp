...
namespace
{
    bool isAdjacent(const GameBoard::Position f, const GameBoard::Position s)
    {
        ...
        if(f.first == s.first) {
	        const int distance = calcDistance(f.second, s.second);
            if(distance == 1) {
                result = true;
            }
        }
        else if(f.second == s.second) {
	        const int distance = calcDistance(f.first, s.first);
            if(distance == 1) {
                result = true;
            }
        }
...
bool GameBoard::move(int index)
{
    ...
    const auto hiddenElementIterrator = std::find(m_rawBoard.begin(), m_rawBoard.end(), boardSize());
...
bool GameBoard::isBoardValid() const
{
    ...
    for (size_t i = 0; i < m_boardSize; ++i) {
        ...
        {
	        constexpr size_t start_point = 1;
